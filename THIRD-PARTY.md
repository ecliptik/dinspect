...
dinspect itself is released under CC0 1.0 Universal (public domain
dedication) — see [LICENSE](LICENSE). This file documents every place
non-dinspect material was consulted during development, and why none
...
dinspect's PicoGUS support was written from those documented protocol
...
BIOS Extensions (VBE) structure offsets used elsewhere in dinspect,
...
dinspect as a result.

## doskutsu (design reference only)

dinspect is a sibling project to doskutsu, whose `setup/profile.c`
implements broadly similar hardware-detection logic (CPU, memory,
video, sound) under doskutsu's own MIT license. Where dinspect's
...
linked into or distributed with `dinspect.exe`. Its license governs
